...
export const promisifyDatabase = (db: sqlite3.Database) => {
  return {
...
export class DatabaseInitializer {
  ...
  private async initTables(): Promise<void> {
    ...
    // 初始化密码表
    await this.db.exec(PasswordsTable.getDDL())

    // 初始化用户表
    await this.db.exec(UsersTable.getDDL())

    // 初始化网站图标表
    await this.db.exec(FaviconsTable.getDDL())
  ...
  static async initializePasswordsDatabase(dbPath: string) {
    return await PasswordsTable.initialize(dbPath)
  }
  ...
  static async initializeFaviconsDatabase(dbPath: string) {
    return await FaviconsTable.initialize(dbPath)
  }
}
...
export class MemoryDatabase {
          ...
          this.db!.get(
            ...
            (err, row: { sql: string } | undefined) => {
              if (err) {
                ...
                // 注意：这里我们不需要 hasGroupColumn 变量，所以删除它
    ...
    return new Promise((resolve, reject) => {
      this.db!.all(
        `SELECT id,
         ...
         FROM passwords
         ORDER BY service`,
        (err, rows: DatabasePasswordEntry[]) => {
          if (err) {
            reject(err)
          } else {
  ...
  async toggleFavorite(id: number): Promise<void> {
    ...
    const row: { is_favorited: number } | undefined = await get(
      ...
      [id]
    ) as { is_favorited: number } | undefined
